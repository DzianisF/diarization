...
export function LocalCompanionDock({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className={embedded ? "" : "fixed bottom-4 left-1/2 z-50 -translate-x-1/2 sm:bottom-6"}>
    {open ? <section aria-label="Use local companion" className="w-[min(92vw,440px)] rounded-2xl border border-[#b49345]/30 bg-[#fffaf0] p-4 shadow-[0_18px_44px_rgba(45,38,18,.18)]">
      ...
      <details className="mt-3 rounded-lg border border-[#b49345]/25 px-3 py-2 text-[11px] leading-4 text-[#66583f]"><summary className="cursor-pointer font-medium text-[#675633]">Comet fallback: save audio locally, then upload it</summary><p className="mt-2">Run <code className="rounded bg-[#f8efd9] px-1 py-0.5">npm run prepare -- --rights-confirmed --output-dir ~/Downloads https://www.youtube.com/watch?v=...</code> in the <code>companion</code> folder. It automatically retries a 403 once with a supported alternate YouTube client. When it finishes, use <strong>Local file</strong> above to select the saved MP3. This path does not need browser-to-localhost access.</p></details>
